...
<div id="filAriane" class="vingt" itemscope="" itemtype="">
            ...
            <a class="nom2" href="javascript:history.back()"><?php echo "Coordinateur SIC " .$query["BdD"]?></a>	
			  	</ul>
			</div>
...
$reponse = $bdd->query('SELECT * FROM contact WHERE Enseigne="'.$query["Enseigne"].'" AND BdD="'.$query["BdD"].'"');

// On affiche chaque entrée une à une
while ($donnees1 = $reponse->fetch())
{
  
  include ('Vue/Formulaires/modifierContactBdD.php')
?>  

<div id="info">
    <div class="bouton" class="cover-container d-flex p-3 mx-auto flex-column col-form-label-lg">
    <button title="Modifier les informations du quartier" class="align-self-sm-end btn" data-bs-toggle="modal" data-bs-target="<?php echo "#edit".$query["BdD"] .$query["Enseigne"]?>" type="button"><img src="../Ressources/images/edit-regular.svg" width="20" /></button>
    </div>
